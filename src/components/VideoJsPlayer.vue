...
export default {
  ...
  methods: {
    ...
    keepOnLive() {
      if (this.config.isAudio) this.playNextVideo();
    },
    bufferNext(player) {
      (async () => {
    ...
    async getNextPlayUrl(n) {
      ...
      for (let k = 0; k < playList.length; k++) {
        try {
          ...
          nextIndex =
            Math.min(playList.length, nextIndex) == playList.length
              ? 0
              : nextIndex;
    ...
    async playListVideo(index) {
      ...
      try {
        ...
        navigator.mediaSession.metadata = new window.MediaMetadata({
          ...
          artwork: [{ src: "icon.png", sizes: "32x32", type: "image/png" }],
        });
      } catch (err) {
        console.error(err);
      }
      ...
      for (let i = 0; i < this.players.length; i++) {
        if (i != this.activeIndex) {
          ...
          document.querySelectorAll(".video-js")[i].style.display = this.config
            .dev
            ? ""
            : "none";
        }
      ...
      try {
        if (actviePlayer.paused()) actviePlayer.currentTime(0);
    ...
    playNextVideo(ended) {
      ...
      if (
        !ended &&
        this.config.isAudio &&
        !this.players[this.activeIndex].paused()
      ) {
        try {
        ...
        if (this.players[this.activeIndex].readyState() < 3) {
          ...
          if (!this.$refs.keeplive.tryTime)
            this.$refs.keeplive.tryTime = new Date().getTime();
          let time =
            (new Date().getTime() - this.$refs.keeplive.tryTime) / 1000;
          let title = "keep live[" + time + "] " + new Date();

          $(this.$refs.keeplive).attr("title", title);

          if (time < timeout / 1000) return;
        } else {
          try {
            this.players[this.activeIndex].play();
          } catch (error) {
      ...
      if (this.config.isLoop > 0 && this.config.isLoop < 4) {
        switch (this.config.isLoop) {
          ...
          case 2:
            break;
          case 3:
            this.players[this.activeIndex].loopCounts++;
            if (
              this.players[this.activeIndex].loopCounts < this.config.loopCount
            ) {
              return this.loopPlay();
            }
        }
      } else if (this.config.isLoop != 4)
      ...
      this.players[this.activeIndex].loopCounts = this.players[
        this.nextIndex
      ].loopCounts = 0;
      ...
      document.querySelectorAll(".video-js")[this.nextIndex].style.display =
        this.config.dev ? "" : "none";
      document.querySelectorAll(".video-js")[this.activeIndex].style.display =
        "";
      document
        .querySelectorAll(".video-js video")
        ...
        this.players = this.$refs.videoPlayer.map((video, index) => {
                ...
                setTimeout(() => {
                  for (var d = 0; d < tts.length; d++) {
                    if (tts[d].kind == "captions")
                      tts[d].mode =
                        self.cc && tts[d].label == "new word" && player.actived
                          ? "showing"
                          : "disabled";
            ...
            setTimeout(() => {
              ...
              for (var d = 0; d < tracks.length; d++) {
                ...
                if (!player.actived || tracks[d].label !== "new word") {
                  console.log(
                    "disable " + player.index + " " + tracks[d].label
                  );
          ...
          player.on("timeupdate", (e) => {
            ...
            player.timer = setTimeout(() => {
              if (
                !player.paused() &&
                this.toInt(player.checkTime) ===
                  this.toInt(player.currentTime())
              ) {
